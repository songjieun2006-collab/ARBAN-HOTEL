(() => {
  ...
  function detailPage(place) {
    ...
    const foodOrCafe = place.category === "food" || place.category === "cafe";
    const heroPhotos = [
      { src: foodOrCafe && place.cutoutImage ? place.cutoutImage : place.image, label: place.imageLabel, cutout: foodOrCafe && !!place.cutoutImage },
      ...(place.extraPhotos || []),
      ...(!foodOrCafe && place.cutoutImage ? [{ src: place.cutoutImage, label: "상품 이미지", cutout: true }] : [])
    ];
      ...
      <article class="place-detail-page">
        ...
        <div class="detail-hero" data-photo-gallery>
          <div class="photo-slides" tabindex="0" role="region" aria-label="${escapeText(place.name)} 사진">
            ${heroPhotos.map(photo => `<div class="photo-slide">${media(photo.src, place.name + " " + photo.label, "detail-photo" + (photo.cutout ? " cutout-photo" : ""))}</div>`).join("")}
          </div>
        ...
        <section class="place-speech" aria-label="${escapeText(guideName)}의 한마디">
          ...
          <div class="place-speech-copy">
            ...
            <p class="place-speech-bubble">${readableText(place.speech || place.description)}</p>
          </div>
        </section>
        ...
        <section class="place-description-panel" aria-labelledby="place-information-title">
          ...
          ${place.description ? `<p class="place-overview">${readableText(place.description)}</p>` : ""}
  ...
  function customCoursePage(course) {
    return `
      ...
      <div class="guide-row custom-guide">
        ...
        <p class="bubble">${readableText(course.speech)}</p>
      </div>
